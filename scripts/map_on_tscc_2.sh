...
if [ "$CMD" == "send" ]; then
    ...
    scp -r $DATA_DIR/processed $BIOWHAT_USER@glassome.ucsd.edu:$GET_DIR 
    exit
else
    if [ "$CMD" == "clean" ]; then
        if [ "$DATA_DIR" == "" ]; then
		...
		if [ "$CMD" == "STAR" ]; then
			OP="perl ${EXEC_DIR}/misc/map-star.pl ${GENOME} "
			WTIME="3:00:00"
			NODES="nodes=1:ppn=8"
		else
	   ...
	   scp -r $BIOWHAT_USER@glassome.ucsd.edu:$GET_DIR $TO_DIR 
        fi
